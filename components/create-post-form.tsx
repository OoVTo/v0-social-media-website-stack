    ...
    try {
      ...
      if (media.length > 0) {
        ...
        for (const file of media) {
          const path = `posts/${user.id}/${postData.id}/${Date.now()}-${file.name}`
          ...
          if ("url" in result) {
            ...
            // Store media upload info in database
            ...
            if (mediaError) {
              console.warn("⚠️ Failed to record media in database:", mediaError.message)
            } else {
